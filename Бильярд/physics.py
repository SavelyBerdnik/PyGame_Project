...
def distance_less_equal(p1, p2, dist):
    distance_different = p1 - p2
    return (distance_different[0] ** 2 + distance_different[1] ** 2) <= dist ** 2
...
def line_ball_collision_check(line, ball):
    if distance_less_equal(line.middle, ball.pos, line.length / 2 + config.ball_radius):
        ...
        return -config.ball_radius / 3 <= projected_distance <= np.hypot(
            *(displacement_to_second_point)) + config.ball_radius / 3 and np.hypot(
            *(closest_line_point - ball.pos + line.line[0])) <= config.ball_radius and np.dot(
...
def collide_line_ball(line, ball):
    ...
    perpendicular_vector = np.array(
        [-normalised_point_diff_vector[1], normalised_point_diff_vector[0]])
    ball.velocity -= 2 * np.dot(perpendicular_vector, ball.velocity) * perpendicular_vector * 0.5 * (
